...
class ReparamLargeKernelConv(nn.Module):
    ...
    def fuse(self):
        ...
        # Add quantization-specific fusion
        if self.is_quantized:
            torch.ao.quantization.fuse_modules(
                self, 
                [['lk_conv', 'lk_bias'], ['sk_conv', 'sk_bias']], 
                inplace=True
        )
...
class GatedFFN(nn.Module):
    ...
    def __init__(self, in_features: int, hidden_features: int = None, out_features: int = None, drop: float = 0.):
        ...
        # quantization
        self.quant_mul = torch.nn.quantized.FloatFunctional()  # Initialize once
        self.is_quantized = False  # Default flag
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Conditional multiplication because of quant
        x = self.quant_mul.mul(activated, main) if self.is_quantized else self.act(gate) * main # The gating mechanism: activated gate values element-wise multiply the main path values.
...
class aether(nn.Module):
    ...
    def prepare_qat(self, opt: Dict[str, Any]):
        ...
        # Propagate quantization flag to all relevant modules
