...
def convert_model(
    ...
    fp16_model = model.half() # Convert the already fused FP32 model to FP16
    ...
    try:
        # Before converting to quantized, the model needs to be prepared for QAT.
        # This will add FakeQuantize modules and set the qconfig.
        # We assume the loaded model is suitable for post-training quantization or was QAT-trained.
        ...
        model_for_int8_conversion = model # Use the already fused FP32 model instance
    ...
    try:
        torch.onnx.export(
            model, # Use the fused FP32 PyTorch model for ONNX export
            dummy_input_fp32,
            fp32_onnx_path,
            export_params=True,
    ...
    try:
        model_fp16_onnx = model.half() # Convert the fused FP32 model to FP16 for this specific ONNX export
