...
# It includes support for structural re-parameterization (fusion) and Quantization-Aware Training (QAT).
...
# These are essential for PyTorch's native quantization workflow.
...
# These imports are fundamental for neosr to discover, configure,
# and interact with the AetherNet model based on the training/inference setup.
...
# This ensures consistency between the dataset's scale and the model's expected output resolution.
# net_opt() typically returns (upscale_factor, unused_parameter).
...
class aether(nn.Module):
    ...
    It is designed for seamless integration with the `neosr` training framework.
    ...
    Args:
        ...
        scale (int): The super-resolution upscale factor (e.g., 2, 3, 4).
                     This parameter is critical for `neosr` as it's passed from the global config
                     and used by the `Upsample` module. Defaults to `upscale_opt` from `net_opt()`.
        ...
        **kwargs: Catches any additional keyword arguments passed from the `neosr` configuration
                  that are not explicitly defined in the `__init__` signature, ensuring compatibility
    ...
    def __init__(
        ...
        in_chans: int = 3,
        embed_dim: int = 128,
        depths: Tuple[int, ...] = (6, 6, 6, 6, 6, 6), # Default to AetherNet-Medium depths
        mlp_ratio: float = 2.0,
        drop_rate: float = 0.,
        ...
        **kwargs: Any, # Catch-all for extra config parameters
    ):
        super().__init__()
...
@ARCH_REGISTRY.register()
def aether_small(**kwargs: Any) -> 'aether':
    """
    AetherNet Small variant, optimized for smaller models and lower resource consumption.
    
    Args:
        **kwargs: Additional parameters passed from the `neosr` configuration.
                  These can override default values defined here or in the `aether` class.
                  
    Returns:
        aether: An instance of the AetherNet Small model.
    """
    return aether(
        embed_dim=96,           # Smaller embedding dimension for a compact model.
        depths=(4, 4, 4, 4),    # Fewer AetherBlocks for reduced depth.
        mlp_ratio=2.0,          # Standard MLP ratio.
        **kwargs                # Pass along other parameters like 'scale', 'in_chans', etc.
    )

@ARCH_REGISTRY.register()
def aether_medium(**kwargs: Any) -> 'aether':
    """
    AetherNet Medium variant, providing a balanced choice between performance and resource usage.
    This is often the default or recommended variant for many general-purpose applications.
    
    Args:
        **kwargs: Additional parameters passed from the `neosr` configuration.
                  
    Returns:
        aether: An instance of the AetherNet Medium model.
    """
    return aether(
        embed_dim=128,              # Balanced embedding dimension.
        depths=(6, 6, 6, 6, 6, 6),  # Medium depth with more AetherBlocks.
        mlp_ratio=2.0,              # Standard MLP ratio.
        **kwargs                    # Pass along other parameters.
    )

@ARCH_REGISTRY.register()
def aether_large(**kwargs: Any) -> 'aether':
    """
    AetherNet Large variant, designed for higher capacity and potentially better super-resolution results.
    This variant consumes more resources but aims for peak quality.
    
    Args:
        **kwargs: Additional parameters passed from the `neosr` configuration.
                  
    Returns:
        aether: An instance of the AetherNet Large model.
    """
    return aether(
        embed_dim=180,                  # Larger embedding dimension.
        depths=(8, 8, 8, 8, 8, 8, 8, 8), # Deeper network with more AetherBlocks.
        mlp_ratio=2.5,                  # Higher MLP ratio for increased expressiveness.
        lk_kernel=13,                   # Uses a slightly larger large-kernel size for more global context.
        **kwargs                        # Pass along other parameters.
    )
